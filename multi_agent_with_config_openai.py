"""
Multi-Agent with GenieAgent and VectorSearch using OpenAI SDK for Databricks

This extends the genie_agent_with_config to also support Vector Search Retriever tools.
...
############################################
# Enhanced YAML Configuration with Vector Search
############################################

CONFIG_YAML = '''genie_agents:
- agent_name: GENIE_REXEL
  description: Agent for GENIE_REXEL analytics
  ...
  tool_description: The agent provides advanced business analytics across products,
    ...
    and assesses competitor impact.
  tool_name: genie_rexel_query_tool

vector_search_tools:
- index_name: agents_lab.product.product_docs_index
  tool_name: search_product_docs
  tool_description: Search for product documentation and technical specifications
  num_results: 3
  filters: null
...
class VectorSearchConfig:
    """Configuration for a Vector Search tool"""
    index_name: str
    tool_name: str
    tool_description: str
    num_results: int = 3
...
class AgentConfig:
    ...
    vector_search_tools: List[VectorSearchConfig] = field(default_factory=list)
...
def create_vector_search_tool_info(vs_config: VectorSearchConfig) -> ToolInfo:
    """Create a Vector Search tool from configuration"""
    # Create the VectorSearchRetrieverTool
    vs_tool = VectorSearchRetrieverTool(
        index_name=vs_config.index_name,
        tool_name=vs_config.tool_name,
        tool_description=vs_config.tool_description,
        num_results=vs_config.num_results,
    )
    
    # Get the tool spec
    tool_spec = vs_tool.tool.copy()
    tool_spec["function"].pop("strict", None)
    
    return ToolInfo(
        name=vs_config.tool_name,
        spec=tool_spec,
        exec_fn=vs_tool.execute
    )

def create_vector_search_tools(vs_configs: List[VectorSearchConfig]) -> List[ToolInfo]:
    """Create multiple Vector Search tools from configurations"""
    tools = []
    for config in vs_configs:
        tools.append(create_vector_search_tool_info(config))
    return tools
...
SYSTEM_PROMPT = """
You are an expert data analyst with access to multiple data sources:
1. **Genie agents** - for structured data analytics and SQL queries
2. **Vector search** - for unstructured documents and text-based information

When analyzing queries:
- Use **Genie agents** for quantitative analysis, metrics, trends, and structured data
- Use **vector search** for documentation, specifications, guides, and text-based knowledge
...
2. "extractedData": array of objects with "label" and "value" properties (for Genie results)
...
6. "insights": string - FORMATTED AS MARKDOWN : detailed analysis of the data patterns + business strategy recommendations
7. "xAxisLabel": string - label for x-axis (if applicable)
8. "yAxisLabel": string - label for y-axis (if applicable)
9. "hasText": boolean - whether text/document data was found from vector search
10. "vectorSearchResults": string - FORMATTED AS MARKDOWN : summary of relevant documents and text findings (if vector search was used)

Rules:
- Extract ALL numerical values with their context from Genie results
...
- Summarize vector search results in a clear, structured way
- Combine insights from both data sources when applicable
...
class ToolCallingAgent(ResponsesAgent):
    """
    Tool-calling Agent with GenieAgent and VectorSearch support
    """
...
def create_agent_with_config(config: AgentConfig) -> ToolCallingAgent:
    ...
    if config.vector_search_tools:
        vs_tools = create_vector_search_tools(config.vector_search_tools)
...
DEFAULT_CONFIG = AgentConfig(
    ...
    vector_search_tools=[VectorSearchConfig(**vs) for vs in config_data.get('vector_search_tools', [])],
...
def create_agent(genie_space_ids: Optional[List[str]] = None, 
                ...
                vector_search_indexes: Optional[List[str]] = None,
                vector_search_tool_names: Optional[List[str]] = None,
                vector_search_descriptions: Optional[List[str]] = None,
    ...
    Args:
        ...
        tool_names: List of tool names for Genie agents
        tool_descriptions: List of tool descriptions for Genie agents
        vector_search_indexes: List of vector search index names
        vector_search_tool_names: List of tool names for vector search
        vector_search_descriptions: List of descriptions for vector search tools
    ...
    if genie_space_ids is not None or vector_search_indexes is not None:
        # Create custom genie agents
        genie_agents = []
        if genie_space_ids:
        ...
        # Create custom vector search tools
        vs_tools = []
        if vector_search_indexes:
            for i, index_name in enumerate(vector_search_indexes):
                vs_tool_name = vector_search_tool_names[i] if vector_search_tool_names and i < len(vector_search_tool_names) else f"search_{i}"
                vs_description = vector_search_descriptions[i] if vector_search_descriptions and i < len(vector_search_descriptions) else f"Search index {i}"
                
                vs_tools.append(VectorSearchConfig(
                    index_name=index_name,
                    tool_name=vs_tool_name,
                    tool_description=vs_description,
                    num_results=3
                ))
        
        config = AgentConfig(
            genie_agents=genie_agents,
            vector_search_tools=vs_tools,
...
#     "input": [
#         {"role": "user", "content": "What are the top revenue categories and where can I find product documentation?"}
#     ]
# })
# print(response)
